...
case "$COMMAND" in
    ...
    help|--help|-h|"")
        echo ""
        echo -e "${BOLD}${CYAN}Colony Session Manager${RESET}"
        echo ""
        echo -e "${BOLD}Usage:${RESET}"
        echo -e "  bash scripts/colony.sh start              Start dev session (interactive tunnel choice)"
        echo -e "  bash scripts/colony.sh start --cloudflare Start Cloudflare tunnel directly"
        echo -e "  bash scripts/colony.sh start --direct     Start with direct public IP"
        echo -e "  bash scripts/colony.sh start --tailscale  Start with Tailscale"
        echo -e "  bash scripts/colony.sh stop               Stop the tunnel daemon"
        echo -e "  bash scripts/colony.sh restart            Stop then start"
        echo -e "  bash scripts/colony.sh status             Full visual health check"
        echo -e "  bash scripts/colony.sh logs               Tail Cloudflare tunnel logs"
        echo -e "  bash scripts/colony.sh ip                 Show all network addresses"
        echo -e "  bash scripts/colony.sh sync [\"message\"]   Commit and push to both repos"
        echo ""
        echo -e "${BOLD}First-time setup:${RESET}"
        echo -e "  bash scripts/colony-setup.sh              Install Docker, Coolify, Supabase"
        echo -e "  bash scripts/colony-setup.sh --cloudflare Wire Cloudflare tunnel (run once)"
        echo -e "  bash scripts/colony-setup.sh --tailscale  Set up Tailscale (optional)"
        echo ""
        ;;
    *)
